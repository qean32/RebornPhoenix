...
export const Users: React.FC<Props> = ({ swap }: Props) => {
    ...
    const [users, loading] = useRequest<userInterface[]>(() => communityService.SEARCH_USERS(search), [...RQKEYFACTORY.searchUser(), search])
    const [myusers] = useRequest<userInterface[]>(() => communityService.GET_USERS_BY_ARRAY(session?.users ?? ''), [...RQKEYFACTORY.findByArrayUser(), session.users])
