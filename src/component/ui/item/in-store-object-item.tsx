...
export const InStoreObjectItem: React.FC<objectDto> = (item: objectDto) => {
    return (
        <div
            ...
            data={JSON.stringify({ ...item, isObject: true, position: { x: 0, y: 0 }, path: `${process.env.SERVER_HOST}storage/${item.path}` })}
        >
            <div className="h-full w-1/2 bg-img bg-shadow transition-300 rounded-sm" style={{ backgroundImage: `url(${process.env.SERVER_HOST}storage/${item.path})` }}></div>
