...
export const InStoreEntityItem: React.FC<entityDto> = (item: entityDto) => {
    return (
        <div
            ...
            data={JSON.stringify({ ...item, isEntity: true, position: { y: 0, x: 0 }, path: `${process.env.SERVER_HOST}storage/${item.path}` })}
        >
            <div className="w-[60px] aspect-square h-fit bg-img transition-300 rounded-full" style={{ backgroundImage: `url(${process.env.SERVER_HOST}storage/${item.path})` }}></div>
