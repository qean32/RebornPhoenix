...
export const ObjectMoreDetailed: React.FC<Props> = ({ }: Props) => {
    const { bestiary, session: { mapsData, currentMap: { id } } } = useAppSelector(state => state.session)
    ...
    const object =
        key == 'more-entity' ?
            ...
            :
            key == 'more-character' ?
                mapsData[id ?? "null"].characters.find(item => item.id == tmpObject?.id)
                :
                mapsData[id ?? "null"].objects.find(item => item.id == tmpObject?.id)
